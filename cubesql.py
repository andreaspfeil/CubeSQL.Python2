...
#                                                          
#  Product:     cubeSQL.Python2 - cubeSQL JSON database driver for Python2
...
class CubeSQL:
  ...
  def open( self, host, username, password ):
    if self.connect( host ) == True:
      ...
      username = sha1( randpool + username )
      password = sha1( randpool + base64.b64encode( sha1( sha1( password, True ), True ) ) )
